function d = ImplementThresholdQAQC(din, dt, para, dpth, var)
% 
...
for MM = 1:12
    isus = find(month(dt)==MM & ...
        (din<para.(dpth).(var){'bd_1',MM} | din>para.(dpth).(var){'bd_99',MM}));
    if ~isempty(isus)
        d(isus) = 3;
    end

    ifail = find(month(dt)==MM & ...
        (din<para.(dpth).(var){'min_val',MM} | din>para.(dpth).(var){'max_val',MM} | isnan(din)));
    if ~isempty(ifail)
        d(ifail) = 4;
    end
end
